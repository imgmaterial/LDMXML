...
train_dir = '/projects/hep/fs9/shared/ldmx/users/pa8701os/LDMXML/8mil_processing/CNNEcal'#This is wherever you have saved the data that was produced after preprocessing that you want to feed to your network  

subdirs, dirs, files = os.walk('/projects/hep/fs9/shared/ldmx/users/pa8701os/LDMXML/8mil_processing/CNNEcal').__next__()#This will walk around the directory and pick out any files it discovers there 
...
class My_Custom_Generator(keras.utils.Sequence):#This is a generator that will load only the batch size number of files per iteration rather than loading alll of the data at once 
  ...
  def __getitem__(self, idx) :#This function actually loads the data 
    ...
    for file_name in batch_x:#This will loop through the files included in the batch 
        A = np.load('/projects/hep/fs9/shared/ldmx/users/pa8701os/LDMXML/8mil_processing/CNNEcal/' + str(file_name))#This assumes that the data is some file that can be directly loaded using np.load
    ...
    ADone = ADone.reshape(ADone.shape[0],34,7,450,1) 
...
model.add(layers.Conv3D(64,(2,2,2),activation='relu',input_shape=(34,7,450,1)))#This adds a 3D convolutional layer 
...
model.add(layers.Dense(64,'tanh',kernel_regularizer=regularizers.L1(l1=1e-3))) #This adds an L1 regularizer in a dense layer thsat is just a series of connected nodes 
...
checkpoint = ModelCheckpoint(filepath='Checkpoints/CNNEcal8mil1ep',save_best_only=True, monitor='accuracy', mode='max', save_freq=1000, verbose=1) 
...
model.save("Models/CNNEcal8mil1ep")#This saves the model, you can train and then stop and then resume the training just have to reload the model 
