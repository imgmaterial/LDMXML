                ...
                if nTS<nc:
                    ...
                    if nc==0:
                        for j in range(n):
                            ...
                            for i in range(156):
                                if i == 0:
                                    B[i] =  (int(chunkTS['{}'.format('x')][j])*1000000 +int(chunkTS['{}'.format('y')][j])*1000+1)#The onlyvariation is that we use the 'x' coordinate as well as the 'y'
                                else:
                        ...
                        for j in range(nc*n,nc*n+NRR):
                            ...
                            for i in range(156):
                                if i == 0:
                                    B[i] =  (int(chunkTS['{}'.format('x')][j])*1000000 +int(chunkTS['{}'.format('y')][j])*1000+1)
                                else:
                                    D = chunkC['{}'.format(i-1)][j]
                                    if pd.isna(D) == False and pd.isna(chunkE['{}'.format(i)][j])==False:
                                            DD = int(D)
