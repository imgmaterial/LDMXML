                ...
                if nTS<nc:
                    ...
                    if nc==0:
                        for j in range(n):
                            ...
                            B = np.array(B)
                            np.savez_compressed('/Directory/RNN/ETSX/4e{}.npz'.format(nc), B)#Saves the array to a file named for example 1e1.npz in the NPZ file format.
                    elif NRR <n:
                        for j in range(nc*n,nc*n+NRR):
                            ...
                            for i in range(450):
                                    ...
                                    if pd.isna(D) == False and pd.isna(chunkE['{}'.format(i)][j])==False:
                                            ...
                                            B[i+4] = (L+1)*10000000+1000000*(M)+1000*C+chunkE['{}'.format(i)][j]
                            B = np.array(B)
                            np.savez_compressed('/Directory/RNN/ETSX/4e{}.npz'.format(nc), B)
                    else:
                        for j in range(nc*NRR,(nc+1)*NRR):
                            ...
                            for i in range(450):
                                    ...
                                    if pd.isna(D) == False and pd.isna(chunkE['{}'.format(i)][j])==False:
                                            ...
                                            B[i+4] = (L+1)*10000000+1000000*(M)+1000*C+chunkE['{}'.format(i)][j]
                            B = np.array(B)
                            np.savez_compressed('/Directory/RNN/ETSX/3e{}.npz'.format(nc), B)
                    nTS = nTS+1
            nE = nE+1
    nc = nc+1
