...
train_dir = '/projects/hep/fs9/shared/ldmx/users/pa8701os/LDMXML/8mil_processing/RNNEcal'#This is wherever you have saved the data that was produced after preprocessing that you want to feed to your network  

subdirs, dirs, files = os.walk('/projects/hep/fs9/shared/ldmx/users/pa8701os/LDMXML/8mil_processing/RNNEcal').__next__()#This will walk around the directory and pick out any files it discovers there 
...
class My_Custom_Generator(keras.utils.Sequence):#This is a generator that will load only the batch size number of files per iteration rather than loading alll of the data at once 
  ...
  def __getitem__(self, idx) :#This function actually loads the data 
    ...
    for file_name in batch_x:#Loads each file with the filenames included 
        A = np.load('/projects/hep/fs9/shared/ldmx/users/pa8701os/LDMXML/8mil_processing/RNNEcal/' + str(file_name))
        AA = A['arr_0']
        Array.append(AA)
...
lstm_3 = keras.layers.GRU(32, activation='tanh',recurrent_dropout=0)(lstm_e)
...
adam = Adam(learning_rate= 0.0001) #Defines the optimizer with the learning rate 
...
checkpoint = ModelCheckpoint(filepath='Checkpoints/RNNEcal8mil1ep',save_best_only=True, monitor='accuracy', mode='max', save_freq=1000, verbose=1) 
...
model.save("Models/RNNEcal8mil1")#Saves the model and can resume training after stopping 
