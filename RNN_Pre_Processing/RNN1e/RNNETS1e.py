...
def RNN_ETS_1e(ecal_path:str, energy_path:str, tspos_path:str ,output_path:str, n:int = 1000, nc:int = 0):
                    ...
                    if nTS<nc:
                        ...
                        if nc==0:
                            for j in range(n):
                                ...
                                for i in range(156):
                                    if i == 0:
                                        B[i] =  (int(chunkTS['{}'.format('Y1')][j])*1000+1)#The 'y' data from the TS is treated as the cell number in our formula 
                                    else:
                            ...
                            for j in range(nc*n,nc*n+NRR):
                                ...
                                for i in range(156):
                                    if i == 0:
                                        B[i] =  (int(chunkTS['{}'.format('Y1')][j])*1000+1)
                                    else:
                                        D = chunkC['{}'.format(i-1)][j]
                                        if pd.isna(D) == False and pd.isna(chunkE['{}'.format(i)][j])==False:
                                                DD = int(D)
