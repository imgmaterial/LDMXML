...
def Process_POS_3_events(input_file:str, output_directory:str):
    ...
    for BB in Y2:#This goes through the Y2 values and sorts them into 24 different categories
        if BB <= -33.0:
            ...
            YT2.append(23)
    for BB in Y3:#This goes through the Y2 values and sorts them into 24 different categories
        if BB <= -33.0:
            YT3.append(0)
        elif -33.0<BB <= -30.0:
                YT3.append(1)
    ...
    P = {"X1": XT1,"Y1":YT1,"X2":XT2, "Y2":YT2,"X3":XT3, "Y3":YT3}#Creates a dictionary of the data

    PDF = pd.DataFrame(P)#Creates a Data Frame

    PDF.to_csv(output_directory+"TSPOS3.csv", index=False)
