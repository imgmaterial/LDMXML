...
readerC = pd.read_csv("/Directory/EcalIDFile4.csv", chunksize=n) #This assumes that the file containing the EcalIDs is too large to load into RAM if not then remove the chunking of the csv
...
for chunkC in readerC:#Will iterate over the chunks of the read csv file
    ...
    for chunkE in readerE:
        ...
        if nE<nc:
            ...
            readerTS = pd.read_csv("TSPOS4.csv", chunksize=n)#Once again assuming that the TS file is too large
                ...
                if nTS<nc:
                    ...
                    if nc==0:
                        for j in range(n):
                            ...
                            MNN["y"] = 3
                            torch.save(MNN,'/Directory/GNN/ETS/4e{}.pt'.format(nc))
                            with zipfile.ZipFile('/Directory/GNN/ETS/4e{}.zip'.format(nc), 'w', zipfile.ZIP_DEFLATED) as f:
                                    f.write('/Directory/GNN/ETS/4e{}.pt'.format(nc))
                            os.remove('/Directory/GNN/ETS/4e{}.pt'.format(nc))
                    elif NRR <n:
