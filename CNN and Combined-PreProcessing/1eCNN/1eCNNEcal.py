        ...
        if nE<nc: #The chunking is done with an iterator that does not act as a list so to ensure that chunkC and chunkE contain the same datapoints we count nE to ensure it becomes equal with nc that counts which iteration chunkC is on.
            ...
            if nc == 0:
                for j in range(n):
                    ...
                    B = np.array(B)
                    np.savez_compressed('/Directory/CNN/Ecal/1e{}.npz'.format(nc), B)#Saves the array to a file named for example 1e1.npz in the NPZ file format, if other file formats are used be sure to change the loading in the training files to be compliant.
                  ...
                  for j in range(nc*n,nc*n+NRR):
                            ...
                            B = np.array(B)
                            np.savez_compressed('/Directory/CNN/Ecal/1e{}.npz'.format(nc), B)
            else:
