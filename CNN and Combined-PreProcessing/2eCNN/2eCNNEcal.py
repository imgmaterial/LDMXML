        ...
        if nE<nc:
            ...
            if nc == 0:
                for j in range(n):
                    ...
                    B = np.array(B)
                    np.savez_compressed('/Directory/CNN/Ecal/2e{}.npz'.format(nc), B)#Saves the array to a file named for example 1e1.npz in the NPZ file format.
            elif NRR <n:
                  for j in range(nc*n,nc*n+NRR):
                            ...
                            for i in range(250):
                                    ...
                                    if pd.isna(D) == False and pd.isna(chunkE['{}'.format(i)][j])==False:
                                        ...
                                        B[L][M][C] = chunkE['{}'.format(i)][j]
                            B = np.array(B)
                            np.savez_compressed('/Directory/CNN/Ecal/2e{}.npz'.format(nc), B)
            else:
