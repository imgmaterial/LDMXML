...
train_dir = '/projects/hep/fs9/shared/ldmx/users/pa8701os/LDMXML/8mil_processing/CNNETSXFixed'#This is wherever you have saved the data that was produced after preprocessing that you want to feed to your network  

subdirs, dirs, files = os.walk('/projects/hep/fs9/shared/ldmx/users/pa8701os/LDMXML/8mil_processing/CNNETSXFixed').__next__()#This will walk around the directory and pick out any files it discovers there 
...
class My_Custom_Generator(keras.utils.Sequence):#This is a generator that will load only the batch size number of files per iteration rather than loading alll of the data at once 
  ...
  def __getitem__(self, idx) :#This function actually loads the data 
    ...
    for file_name in batch_x:#This will loop through the files included in the batch 
        A = np.load('/projects/hep/fs9/shared/ldmx/users/pa8701os/LDMXML/8mil_processing/CNNETSXFixed/' + str(file_name))#This assumes that the data is some file that can be directly loaded using np.load
...
checkpoint = ModelCheckpoint(filepath='Checkpoints/CNNETSX8mil1ep',save_best_only=True, monitor='accuracy', mode='max', save_freq=1000, verbose=1) 
#Loss Function: CategoricalCrossentropy
...
model.save("Models/CNNETSX8mil1ep")
