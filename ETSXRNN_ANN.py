...
train_dir = 'Output/Pre-Processing/RNN/ETSX'#This is wherever you have saved the data that was produced after preprocessing that you want to feed to your network  

subdirs, dirs, files = os.walk('Output/Pre-Processing/RNN/ETSX').__next__()#This will walk around the directory and pick out any files it discovers there 
...
class My_Custom_Generator(keras.utils.Sequence):#This is a generator that will load only the batch size number of files per iteration rather than loading alll of the data at once 
  ...
  def __getitem__(self, idx) :#This function actually loads the data 
    ...
    for file_name in batch_x:#Loads each file with the filenames included 
        A = np.load('Output/Pre-Processing/RNN/ETSX/' + str(file_name))
        AA = A['arr_0']
        Array.append(AA)
